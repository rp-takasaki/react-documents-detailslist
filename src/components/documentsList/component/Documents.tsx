...
export default class LibraryDocuments extends React.Component<
  IDocumentsProps,
  IDocumentsState
> {
  ...
  constructor(props: IDocumentsProps) {
    ...
    this._onContextualMenuDismissed =
      this._onContextualMenuDismissed.bind(this);
              ...
              <div className={styles.topBarFilters}>
                {this.state.showResetFilters && (
                  <span className={styles.resetFilter}>
                    <i
                      className="ms-Icon ms-Icon--ClearFilter"
                      aria-hidden="false"
                      role="button"
  ...
  public ClickFilter(
    ...
    if (item) {
      ...
      columns
        .filter((matchColumn) => matchColumn.key === item.data)
      ...
      let newDocs = [];
      if (item.data != "Tags") {
        newDocs = documents.filter(
          (matchDoc) => matchDoc[item.data] === item.key
        );
      } else {
  ...
  private _getContextualMenuProps(
    ...
    debugger;
    let utility = new Utils();
    let items: IContextualMenuItem[] = utility.GetSortingMenuItems(
      column,
      this._onSortColumn
    );
  ...
  private _GetFilterValues(column: IColumn): IContextualMenuItem[] {
    debugger;
    let utility = new Utils();
    let filters = utility.GetFilterValues(
      column,
      this.state.displayedDocuments,
      this.ClickFilter
    );
    return filters;
  }
