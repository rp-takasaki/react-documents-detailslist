...
export default class SharePointDataProvider implements IDataProvider {
  ...
  public readDocumentsFromSearch(): Promise<IDocument[]> {
    ...
    let searchQuery =
      '(path:"' +
      encodeURIComponent(this._libraryAbsoluteUrl) +
      '*")AND(IsDocument:1)';
    let webAbsoluteUrl = this._webPartContext.pageContext.web.absoluteUrl;
    const searchRequestUrl1: string =
      `${webAbsoluteUrl}/_api/search/query?querytext='${searchQuery}'` +
    ...
    return this._webPartContext.spHttpClient
      .get(searchRequestUrl1, SPHttpClient.configurations.v1, {
        headers: {
          "odata-version": "3.0",
  ...
  public readDocumentsFromLibrary(): Promise<IDocument[]> {
    debugger;
    let utility = new Utils();
    let libraryRelativeUrl = utility.GetRelativePathFromAbsolute(
      this._libraryAbsoluteUrl
    );

    return this._readListId(libraryRelativeUrl).then(
      (listId: string): Promise<IDocument[]> => {
        ...
        /*
            ...
            */
        return this._webPartContext.spHttpClient
          .get(queryUrlGetAllItems, SPHttpClient.configurations.v1)
          .then((response: any) => {
            if (response.status >= 200 && response.status < 300) {
              return response.json();
            } else {
              ...
              for (let i = 0; i < data.value.length; i++) {
                ...
                if (item.ContentTypeId.indexOf("0x0101") == 0) {
                  var doc: IDocument = {
                    ...
                    FileIcon:
                      item.ContentType.Name != "Folder"
                        ? utility.GetImgUrlByFileExtension(item.DocIcon)
                        : utility.GetImgUrlByFileExtension("folder"),
                  };
                  documents.push(doc);
                }
              }
          ...
          .catch((ex) => {
            console.log(
              "readDocumentsFromLibrary > spHttpClient.get()...catch:",
              ex
            );
            throw ex;
          });
  ...
  private _readListId(listRelativeUrl: string): Promise<string> {
    let queryUrlGetList =
      ...
      decodeURIComponent(listRelativeUrl) +
      "')/Properties";

    return this._webPartContext.spHttpClient
      .get(queryUrlGetList, SPHttpClient.configurations.v1)
      .then((response: any) => {
        if (response.status >= 200 && response.status < 300) {
          return response.json();
        } else {
