...
export class Utils {
  ...
  public GetRelativePathFromAbsolute(absoluteUrl) {
    var serverRelativeUrl = absoluteUrl
      .toLowerCase()
      .replace(
   ...
   */
  public GetSortingMenuItems(
    column: IColumn,
    onSortColumn: (column: IColumn, isSortedDescending: boolean) => void
  ): IContextualMenuItem[] {
    let menuItems = [];
  ...
  public OpenDocument(
    ...
    try {
      ...
      if (docItem.FileRef.toLowerCase().indexOf(".pdf") > 0) {
        ...
        documentWebUrl =
          docItem.ParentWebUrl +
          "/_layouts/WopiFrame.aspx" +
          "?sourcedoc=" +
          encodeURIComponent("{" + docItem.UniqueId + "}") +
          "&action=default";
      }
   ...
   */
  private _IsValuePresented(
    currentValues: IContextualMenuItem[],
    newValue: string
  ): boolean {
